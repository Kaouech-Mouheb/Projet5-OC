...
.catch(error => {
  ...
  .then((data) =>{
    console.log(data)
    secondProductJs(data[1].name, data[1].imageUrl, data[1].description, data[1].price, data[1].lenses[0], data[1].lenses[1]);
  });
const secondProductJs = (titre, image, description, prix, lenses1, lenses2) =>{
    const product1 = document.getElementById("second-product");
    ...
    const descriptionJs = document.getElementById("second-product_description");
