...
.catch(error => {
  ...
  .then((data) =>{
    console.log(data)
    secondProductJs(data[3].name, data[3].imageUrl, data[3].description, data[3].price, data[3].lenses[0], data[3].lenses[1]);
  });
const secondProductJs = (titre, image, description, prix, lenses1, lenses2) =>{
    const product1 = document.getElementById("foorth-product");
    ...
    const descriptionJs = document.getElementById("foorth-product_description");
